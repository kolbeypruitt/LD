...
- (void)addCustomViews
{
    ...
    const int count = 7;
    NSArray *titleArray = @[@"科室",@"临时坐诊时间",@"临时坐诊地址",@"详细地址",@"医院名称",@"拟邀医师职位",@"是否住院"];
...
- (void)setModel:(TemporaryModel *)model
{
    _model = model;
    ...
    UILabel *time  = [self.contentLabels objectAtIndex:1];
    UILabel *location = [self.contentLabels objectAtIndex:2];
    UILabel *address = [self.contentLabels objectAtIndex:3];
    UILabel *hospital = [self.contentLabels objectAtIndex:4];
    UILabel *jobType = [self.contentLabels objectAtIndex:5];
    UILabel *ishospital = [self.contentLabels objectAtIndex:6];
