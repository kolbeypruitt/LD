...
- (void)addCustomViews
{
    ...
    const int count = 9;
    NSArray *titleArray = @[@"科室",@"待查疾病名称",@"患者病例摘要",@"会诊约定时间",@"会诊约定地址",@"详细地址",@"医院名称",@"拟邀医师职位",@"是否住院"];
...
- (void)setModel:(StubbornModel *)model
{
    _model = model;
    ...
    UILabel *caseAbstract = [self.contentLabels objectAtIndex:2];
    UILabel *time = [self.contentLabels objectAtIndex:3];
    UILabel *location = [self.contentLabels objectAtIndex:4];
    UILabel *address = [self.contentLabels objectAtIndex:5];
    UILabel *hospital = [self.contentLabels objectAtIndex:6];
    UILabel *jobType = [self.contentLabels objectAtIndex:7];
    UILabel *ishospital = [self.contentLabels objectAtIndex:8];
    
    departmentLabel.text = model.department;
    illnessLabel.text = model.illness;
    caseAbstract.text = model.caseAbstract;
