...
@interface AppendRewardView ()<UITextFieldDelegate,UIScrollViewDelegate>
...
@end
@implementation AppendRewardView
...
- (void)addCustomViews
{
    NSArray *labelArray = @[@"悬赏金额",@"姓名",@"身份证号",@"性别",@"最后一次就医医院",@"最后一次就医科室",@"最后一次诊断",@"地点",@"邀请医生的专业",@"邀请医生的职位",@"请医目的",@"VIP",@"备注"];
    NSArray *placeholderArray = @[@"请输入悬赏金额",@"请输入姓名",@"请输入身份证号",@"选择性别",@"请输入",@"请点击选择",@"请输入",@"请点击选择",@"请输入请医地址",@"请输入医生职位",@"请点击选择",@"是否需要VIP",@"请输入备注"];
...
- (AppendInviteParam *)fillParam
{
    ...
    int money = [[[self.textFields objectAtIndex:0] text] intValue];
    NSString *name = [[self.textFields objectAtIndex:1] text];
    NSString *idcardNo = [[self.textFields objectAtIndex:2] text];
    int gender = [[[self.textFields objectAtIndex:3] enterData] gender];
    NSString *lastHospital = [[self.textFields objectAtIndex:4] text];
    NSString *lastDep = [[self.textFields objectAtIndex:5] text];
    NSString *lastDia = [[self.textFields objectAtIndex:6] text];
    NSString *address = [[self.textFields objectAtIndex:7] text];
    NSString *profession = [[self.textFields objectAtIndex:8] text];
    NSString *job = [[self.textFields objectAtIndex:9] text];
    NSString *purpose = [[self.textFields objectAtIndex:10] text];
    int isvip = [[[self.textFields objectAtIndex:11] enterData] isvip];
    NSString *remark = [[self.textFields objectAtIndex:12] text];
    param.money = money;
    param.type = 2;
    param.name = name;
